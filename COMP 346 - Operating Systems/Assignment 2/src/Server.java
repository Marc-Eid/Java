...
public class Server extends Thread {
    ...
    Server(String stid)
    {
    	...
    	else if(serverThreadRunningStatus2 == null)
    	{
    		serverThreadId = stid;							/* unshared variable so each thread has its own copy */
     ...
     public void initializeAccounts()
     {
        ...
        while (inputStream.hasNextLine())
        {
            ...
            catch(InputMismatchException e)
            {
                ...
                Network.writeToOutputFile("Line " + i + "file account.txt invalid input");
                System.exit(0);
            }
            i++;
        }
...
//        		 }
     ...
     */
     ...
     //we used synchronized block because its better for performance, since it has a smaller scope. 
     ...
     */
     ...
     {  
    	 ...
    	 if(getServerThreadId().equals("Server Thread One")) {
    		 ...
    		 System.out.println("\n Terminating server thread One - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    		 Network.writeToOutputFile("\n Terminating server thread One - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");

    		 //set status as terminated 
    		 this.setServerThreadRunningStatus1("terminated");

    	 }else if(getServerThreadId().equals("Server Thread Two")) {
    		 //set status as running
    		 ...
    		 processTransactions(trans);
    		 serverEndTime = System.currentTimeMillis();
    		 System.out.println("\n Terminating server thread Two - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    		 Network.writeToOutputFile("\n Terminating server thread Two - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    	 ...
    	 }else if(getServerThreadId().equals("Server Thread Three")) {
    		 //set status as running
    		 this.setServerThreadRunningStatus3("running");

    		 serverStartTime = System.currentTimeMillis();
    		 processTransactions(trans);
    		 serverEndTime = System.currentTimeMillis();
    		 System.out.println("\n Terminating server thread Three - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    		 Network.writeToOutputFile("\n Terminating server thread Three - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
    	 ...
    	 //just to avoid multiple threads disconnecting. although its not an issue
    	 synchronized (account) {
    		 
    		 if(Network.getServerConnectionStatus().equals("connected") && getServerThreadRunningStatus1().equals("terminated") && getServerThreadRunningStatus2().equals("terminated")
    				) {
